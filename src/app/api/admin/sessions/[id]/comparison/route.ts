...
import { generateRandomScores, runMatching, Student, School } from '@/lib/matching/da-algorithm';
import { runBaselineMatching, summarizeResults } from '@/lib/matching/algorithms/baseline';
import { applySafetyFirstStrategy } from '@/lib/matching/strategy';
...
async function loadModel(sessionId: number) {
  ...
  if (applicationsResult.rows.length === 0) {
    return { students: [] as Student[], schools: [] as School[] };
  }
...
export async function GET(
  ...
  try {
    ...
    const strategicStudents = applySafetyFirstStrategy(students, schools);
    const baseline = runBaselineMatching(strategicStudents, schools, '志望校下げを考慮');
    const baselineSummary = summarizeResults(baseline.results, students);
    const da = runMatching('da', students, schools);
    ...
    return ok({
      ...
      delta: {
        firstChoiceRate: da.summary.firstChoiceRate - baselineSummary.firstChoiceRate,
        unmatchedCount: da.summary.unmatchedCount - baselineSummary.unmatchedCount,
      },
    });
  } catch (error) {
    console.error(`[api/admin/sessions/${id}/comparison][GET] failed:`, error);
    return fail('Failed to fetch comparison', 500, error);
  }
}
